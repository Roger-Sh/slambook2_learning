...
// paths
std::string left_file = "../../../chapter_demo/ch08/data/left.png";
std::string disparity_file = "../../../chapter_demo/ch08/data/disparity.png";
boost::format fmt_others("../../../chapter_demo/ch08/data/%06d.png");  // other files
...
/**
 * @brief class for accumulator jacobians in parallel
 *
 */
...
{
    // read img
    cv::Mat left_img = cv::imread(left_file, 0);            // color left
    cv::Mat disparity_img = cv::imread(disparity_file, 0);  // depth

    // let's randomly pick pixels in the first image and generate some 3d points in the first image's frame
    cv::RNG rng;  // random number generator
    int nPoints = 2000;
    int boarder = 20;
    VecVector2d pixels_ref;         // picked pixel
    std::vector<double> depth_ref;  // picked pixel depth
...
/**
 * @brief accumulate jacobian using cv parallel
 *
 * @param range
 */
...
{
    // parameters
    const int half_patch_size = 1;  // 这里patch比较小, 是否可以增大?
    int cnt_good = 0;
    Matrix6d hessian = Matrix6d::Zero();
    Vector6d bias = Vector6d::Zero();
    double cost_tmp = 0;

    // accumulate jacobian in range
    for (size_t i = range.start; i < range.end; i++)
    {
        /**
         * @brief compute the projection in the second image
         *
         */
        ...
        // check uv in img bound, consider patch size
        ...
        /**
         * @brief compute error and jacobian
         *
         */
