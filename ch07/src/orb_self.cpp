...
#include <string>
#include <nmmintrin.h>  // SSE 指令集相关
#include <chrono>

// global variables
std::string first_file = "../image/1.png";
std::string second_file = "../image/2.png";
...
/**
 * @brief main, test handwrite ORB match
 * 
 * @param argc 
 * @param argv 
 * @return int 
...
{
    // load image
    cv::Mat image_1 = cv::imread(first_file, 0);
    cv::Mat image_2 = cv::imread(second_file, 0);
    assert(image_1.data != nullptr && image_2.data != nullptr);

    // detect FAST keypoints1 using threshold=40, and compute descriptor
    std::chrono::steady_clock::time_point t1 = std::chrono::steady_clock::now();
    std::vector<cv::KeyPoint> keypoints1;
    cv::FAST(image_1, keypoints1, 40);  // 提取FAST角点, 设置阈值40
    std::vector<DescType> descriptor1;
    ComputeORB(image_1, keypoints1, descriptor1);

    // same for the second
    std::vector<cv::KeyPoint> keypoints2;
    std::vector<DescType> descriptor2;
    cv::FAST(image_2, keypoints2, 40);
    ComputeORB(image_2, keypoints2, descriptor2);
    ...
    // plot the matches
    cv::Mat image_show;
    cv::drawMatches(image_1, keypoints1, image_2, keypoints2, matches, image_show);
...
void ComputeORB(const cv::Mat &img, 
    ...
    std::vector<DescType> &descriptors)
{
    const int half_patch_size = 8;  // keypoint patch 8 pixels, 关键点周围8像素距离
    const int half_boundary = 16;   // image edge 16 pixels, bad kp, 图边缘16像素距离
    int bad_points = 0;

    // 遍历检测到的关键点
    for (auto &kp : keypoints)  
    {
        // check kp in bad edge pixels
        ...
        // 计算图像的矩
        float m01 = 0, m10 = 0;
        for (int dx = -half_patch_size; dx < half_patch_size; ++dx)         // -8 < dx < 8
        {
            for (int dy = -half_patch_size; dy < half_patch_size; ++dy)     // -8 < dy < 8
            {
    ...
    std::vector<cv::DMatch> &matches)
{
    ...
    for (size_t i1 = 0; i1 < desc1.size(); i1++)
    {
        ...
        for (size_t i2 = 0; i2 < desc2.size(); i2++)
        {
            ...
            for (int k = 0; k < 8; k++)
            {
                // 使用SSE指令集计算uint变量中1的个数, 从而计算汉明距离
